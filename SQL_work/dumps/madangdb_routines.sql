...
--
-- Temporary view structure for view `vv_orders`
--

DROP TABLE IF EXISTS `vv_orders`;
/*!50001 DROP VIEW IF EXISTS `vv_orders`*/;
SET @saved_cs_client     = @@character_set_client;
/*!50503 SET character_set_client = utf8mb4 */;
/*!50001 CREATE VIEW `vv_orders` AS SELECT 
 1 AS `orderid`,
 1 AS `custid`,
 1 AS `bookid`,
 1 AS `name`,
...
--
-- Final view structure for view `vv_orders`
--
...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`root`@`localhost` SQL SECURITY DEFINER */
/*!50001 VIEW `vv_orders` AS select `o`.`orderid` AS `orderid`,`o`.`custid` AS `custid`,`o`.`bookid` AS `bookid`,`c`.`name` AS `name`,`b`.`bookname` AS `bookname`,`o`.`saleprice` AS `saleprice`,`o`.`orderdate` AS `orderdate` from ((`book` `b` join `orders` `o`) join `customer` `c`) where ((`c`.`custid` = `o`.`custid`) and (`b`.`bookid` = `o`.`bookid`)) */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
...
-- Dump completed on 2024-08-06 15:37:01
